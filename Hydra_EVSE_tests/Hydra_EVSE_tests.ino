...
#define CAR_A_PILOT_SENSE_PIN   2
...
struct persisted_struct {
  ...
  void validate() {
    ...
    for (int i = 0; i < sizeof ( events ); i ++ ) events [i].validate();
...
static void die() {
  ...
  log(LOG_INFO, P("Entered DEAD state"));
  do {
...
static void gfiTestFailure(unsigned char state) {
  log(LOG_INFO, P("GFI self test failure"));
  display.setBacklight(RED);
  display.clear();
...
void setup() {
  ...
  // DEBUG -- don't do this during setup, do that as a unit test.
